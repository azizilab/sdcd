...
def simulate_chain(n, d):
    B_true = np.zeros((d, d))
    B_true[np.triu_indices(d, k=1)] = 1
    B_true[np.triu_indices(d, k=2)] = 0
    xs = [0]
    for i in range(d):
        x = np.random.normal(xs[-1], 5 / (i + 1), size=n)
        xs.append(x)
    X = np.stack(xs[1:], 1)
    return X, B_true
...
def simulate_nonlinear_sem_knockdown(B, n, sem_type, param_dict, knockdown_idx, knockdown_eff = 1.0, noise_scale=None):
    ...
    Args:
        ...
        n (int): num of samples
        sem_type (str): mlp, mim, gp, gp-add
        param_dict (dict): parameters of SEM generated by ``simulate_nonlinear_sem``.
        knockdown_idx (int): which node to knockdown.
        knockdown_eff (float): percent of x to remove (1.0) is a full hard intervention.
        noise_scale (np.ndarray): scale parameter of additive noise, default all ones
    ...
    def _simulate_single_equation(X, scale, params):
        """X: [n, num of parents], x: [n]"""
        z = np.random.normal(scale=scale, size=n)
        pa_size = X.shape[1]
        if pa_size == 0:
