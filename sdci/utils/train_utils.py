...
class AnnDataset(Dataset):
    def __init__(self, anndata, perturbation_colname= 'targets'): # regime_format = True
        ...
        def string_to_binary(row):
            if row == "":
                return np.ones(self.X.shape[1], dtype=int)
            else:
                # convert string indices to int
                indices = set(map(int, row.split(","))) 
                return np.array(
                    [0 if i in indices else 1 for i in range(self.X.shape[1])]
                )

        self.mask = combined_columns.apply(string_to_binary)
        self.mask = torch.LongTensor(
            np.vstack(self.mask.to_numpy())
        )
...
# Remark: for econding gene names, we can also use the method used in crete_intervention_dataset
# This will give us another possible implementation of AnnDataset
class AnnDataset_bis(Dataset):
    def __init__(self, anndata, perturbation_colname= 'targets'): # regime_format = True (according to the task)
        """
        Creating the Dataset object from an anndata object
        
        Args:
            anndata (Anndata): Anndata data structure
            pertubation_colname (str): name of the pertubation column in anndata.obs
        """
        super().__init__()
        # We should create three tensors: X, mask and n_perturbated_genes
        ...
        unstacked_perturbation_columns = (
                anndata.obs[perturbation_colname]
                .map(str)
                # creating a dataframe where each column contains one name of gene
                .str.split(",", expand=True)
                # dropping the column with cell names
                .reset_index(drop=True)
                # associating to each perturbated gene the number of the column that had in the splitting
                .stack()
                # replacing names of genes with their column index in var_names 
                .map(column_mapping)
                .fillna(-1)
                .astype(int)
                .unstack(fill_value=-1)
        ...
        # self.n_perturbated_genes: the number of pertubated genes for each cell
        self.n_perturbated_genes = torch.LongTensor(
            self.X.shape[1] - self.mask.sum(axis=1)
        )
...
def create_anndata_loader_bis(anndata, batch_size, perturbation_colname = "targets", shuffle = True):
    dataset = AnnDataset_bis(anndata, perturbation_colname)
    return DataLoader(dataset, batch_size= batch_size, shuffle= shuffle)
